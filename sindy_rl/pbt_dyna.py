...
def dyna_sindy(config):
    ...
    for n_iter in range(start_iter, train_iterations):
        ...
        if (n_iter % dyn_fit_freq) == dyn_fit_freq - 1:
            (trajs_obs, trajs_acts, trajs_rew) = dyna.collect_data(
        ...
        #   pbt example
        if checkpoint:
            session.report(train_results, checkpoint=checkpoint)
        else:
...
if __name__ == "__main__":
    ...
    filename = os.path.join(
        ...
        "sindy_rl",
        "config_templates",
        "dyna_swimmer_pbt.yml",  # replace with appropriate config yaml
    )
