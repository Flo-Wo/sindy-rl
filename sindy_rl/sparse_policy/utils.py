...
def get_models(trial_dir, check_idx, return_policy=False):
    ...
    data = {"on_pi": pi_data, "off_pi": off_pi_data}
...
def fit_policy(
    ...
    feature_library = PolynomialLibrary(
        degree=poly_deg, include_bias=include_bias, include_interaction=True
    )
    ...
    sparse_policy = SparseEnsemblePolicy(
        optimizer=optimizer,
...
def fit_policies_v(
    ...
    evals = np.array(
        [
            eval_policy(
                policy, X_eval, U_eval, clip_params=clip_params, use_median=use_median
            )
