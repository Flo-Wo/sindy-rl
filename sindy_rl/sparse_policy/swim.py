...
if __name__ == "__main__":
    ...
    swim_dyn, swim_rew, swim_data, swim_policy, swimmer_env = get_models(
        trial_dir, check_idx=1250, return_policy=True
    )
    swim_unbounded_policy = RLlibPolicyWrapper(
        swim_policy.algo.get_policy(), mode="policy"
    )
    ...
    def fit_swim(
        ...
        ic_X = np.random.normal(loc=0, scale=0.2, size=(N_ICs, 8))
    ...
    # and select best one
    swim_res_dict, swim_normal_traj_data = fit_swim(
        swimmer_env,
        swim_dyn,
        swim_policy,
        swim_unbounded_policy,
        hyper_mesh,
        n_dyn_rollout=10000,
        poly_deg=3,
        seed=0,
    ...
    eval_obs, eval_acts, eval_rews = rollout_env(
        swimmer_env.real_env,
    ...
    swim_baseline_obs, swim_baseline_acts, swim_baseline_rews = rollout_env(
        swimmer_env.real_env, policy=swim_policy, n_steps=15000, verbose=True, seed=42
    )
