...
if __name__ == "__main__":
    ...
    cart_unbounded_policy = RLlibPolicyWrapper(
        cart_policy.algo.get_policy(), mode="policy"
    )

    # evaluate baseline
    cart_baseline_obs, cart_baseline_acts, cart_baseline_rews = rollout_env(
        cart_env.real_env, policy=cart_policy, n_steps=15000, verbose=True
    )
    ...
    def fit_cart(
        ...
        N_ICs = 400
        th_eps = 0.1
        th_random = np.random.normal(loc=np.pi, scale=0.1, size=(N_ICs))
        ic_X = np.random.normal(loc=0, scale=0.25, size=(N_ICs, 5))
        ...
        env.reset()
        traj_obs, traj_acts, traj_rews = rollout_env(
            env, policy, n_steps=n_dyn_rollout, n_steps_reset=500, verbose=True
        )
        ...
        noise_scale = 1e-1
        noise = np.random.normal(loc=0, scale=noise_scale, size=X_data.shape)
        X_data = np.concatenate([X_data + noise])
        ...
        traj_U_eval = unbounded_policy.algo.compute_actions(traj_X_eval, explore=False)[
            0
        ]
        ...
        res_dict = fit_policies_v(
            **val,
            hyper_mesh=hyper_mesh,
            n_models=20,
            use_median=use_median,
            clip_params=CLIP_BOUNDS,
            bounds=CLIP_BOUNDS,
            poly_deg=poly_deg,
        )
    ...
    # and select best one
    cart_res_dict, cart_normal_traj_data = fit_cart(
        cart_env,
        cart_dyn,
        cart_policy,
        cart_unbounded_policy,
        hyper_mesh,
        n_dyn_rollout=5000,
        use_median=False,
        poly_deg=3,
    ...
    cart_eval_obs, cart_eval_acts, cart_eval_rews = rollout_env(
        cart_env.real_env, policy=cart_sparse_policy, n_steps=15000, verbose=True
    )
