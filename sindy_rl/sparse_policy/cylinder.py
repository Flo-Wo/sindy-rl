...
if __name__ == "__main__":
    ...
    cyl_dyn, cyl_rew, cyl_data, cyl_policy, cyl_env = get_models(
        trial_dir, check_idx=200, return_policy=True
    )

    cyl_unbounded_policy = RLlibPolicyWrapper(
        cyl_policy.algo.get_policy(), mode="policy"
    )
    ...
    def fit_cyl(
        ...
        if ics == "gauss":
            noise_scale = 0.5
            ic_X = np.random.normal(
                loc=np.zeros(2), scale=noise_scale, size=(n_dyn_rollout, 2)
            )
        elif ics == "uniform":
            low = -1.5
            high = 1.5
            ic_X = np.random.uniform(low=low, high=high, size=(n_dyn_rollout, 2))
        elif ics == "convex":
            ...
            noise_scale = 0.1
            ic_X += np.random.normal(
                loc=np.zeros(2), scale=noise_scale, size=ic_X.shape
            )
        ...
        noise = np.random.normal(loc=np.zeros(2), scale=noise_scale, size=X_data.shape)
    ...
    cyl_res_dict, cyl_normal_traj_data, traj_obs = fit_cyl(
        cyl_env,
        cyl_dyn,
        cyl_policy,
        cyl_unbounded_policy,
        hyper_mesh,
        cyl_data,
        n_dyn_rollout=10000,
        n_reset=10,
