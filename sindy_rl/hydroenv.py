...
class HydroEnvWrapper(hgym.FlowEnv):
    ...
    def __init__(self, env_config=None):
        ...
        self.action_space = Box(low=self.action_space.low, high=self.action_space.high)
    ...
    def real_step(self, action):
        ...
        if self.use_filter:
            ...
            tau = self.flow.TAU
            dt = self.solver.dt
            self.hydro_state = self.prev_hydro_state + (dt / tau) * (
                self.hydro_state - self.prev_hydro_state
            )
    ...
    def reset(self, seed=0, options=None, **reset_kwargs):
        ...
        for i in range(self.n_skip_on_reset):
            self.hydro_state, rew, done, info = self.real_step(
                np.zeros(self.flow.ACT_DIM)
            )
