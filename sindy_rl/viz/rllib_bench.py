    ...
    **data_kwargs,
    ...
    for i, (name, df_list) in enumerate(df_list_dict.items()):
        ...
        T, med_x, min_x, max_x, q25_x, q75_x, mean_x = get_data(
            df_list, plot_key, t_key=t_key, mode=mode, **data_kwargs
        )
    ...
    for ax in axes:
        ax.set_xlabel(
            f"{unit_str} of Interactions in Ground Truth Environment", fontsize=15
        )
    ...
    xticks = left_ticks
    xtick_labels = [rf"{number/unit_factor:.0f}" for number in xticks]
    xtick_labels[0] = "0"
...
def plot_best_comparison_pbt(
    ...
    **data_kwargs,
    ...
    for ax in axes:
        ax.set_xlabel(
            f"{unit_str} of Total Population Interactions in GT Environment",
            fontsize=15,
        )
        ax.tick_params(axis="both", labelsize=15)
        ax.xaxis.get_offset_text().set_fontsize(15)
