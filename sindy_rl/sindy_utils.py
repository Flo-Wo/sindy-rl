...
def lin_and_cube_library(poly_int=False):
    ...
    """
    if poly_int:
        library_functions = [lambda x: x, lambda x, y: x * y**2, lambda x: x**3]
        library_names = [lambda x: x, lambda x, y: f"{x} {y}^2", lambda x: f"{x}^3"]
    else:
        library_functions = [lambda x: x, lambda x: x**3]
        library_names = [lambda x: x, lambda x: f"{x}^3"]

    polyLibCustom = ps.CustomLibrary(
        library_functions=library_functions, function_names=library_names
    )
...
def get_affine_lib(
    poly_deg, n_state=2, n_control=2, poly_int=False, tensor=False, use_cub_lin=False
):
    """
    ...
    if use_cub_lin:
        ...
        polyLib = ps.PolynomialLibrary(
            ...
            library_ensemble=None,
        )
    affineLib = ps.PolynomialLibrary(
        degree=1, include_bias=False, include_interaction=False, library_ensemble=None
    )
    ...
    inputs_per_library = np.array(
        [inputs_state_library, inputs_control_library], dtype=int
    )
...
def get_affine_lib_from_base(base_lib, n_state=2, n_control=2, include_bias=False):
    ...
    Returns library for regression of the form:
        ...
        `n_control` (int):  The dimension of the control variable u
    """
    control_lib = ps.PolynomialLibrary(
...
def build_optimizer(config):
    ...
    if ensemble:
        optimizer = ps.EnsembleOptimizer(opt=base_optimizer, **ensemble)
    else:
...
def build_feature_library(config):
    ...
    if lib_name == "affine":
        ...
        # WARN: this should not be hardcoded --> library_ensemble is deprecated
        lib = lib_class(library_ensemble=None, **lib_kwargs)
    return lib
