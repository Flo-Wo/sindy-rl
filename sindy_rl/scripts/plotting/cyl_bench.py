...
if __name__ == "__main__":
    ...
    cyl_names = {
        "Baseline PPO": "baseline_ppo",
        "SINDy-RL": "sindy_rl",
    }

    cyl_exp_dirs = {
        key: os.path.join(ray_dir, exp_name) for key, exp_name in cyl_names.items()
    }

    cyl_df_list_dict = {key: get_dfs(exp_dir) for key, exp_dir in cyl_exp_dirs.items()}
    cyl_seed_dirs = {
        key: sorted(glob.glob(os.path.join(exp_dir, "*/")))
        for key, exp_dir in cyl_exp_dirs.items()
    }
    ...
    _PLOT_KEYS = {
        "Baseline PPO": "dyn_collect/mean_rew",
        "SINDy-RL": "dyn_collect/mean_rew",
    }

    # which keys to use for the x-axis
    _TIME_KEYS = {
        "Baseline PPO": "num_agent_steps_sampled",  # Direclty from RLLib
    ...
    for i, (name, df_list) in enumerate(df_list_dict.items()):
        ...
        # get quantile data
        T, med_x, min_x, max_x, q25_x, q75_x, mean_x = get_data(
            df_list, plot_key, t_key=t_key, mode=_MODE, win=win
        )
    ...
    ax.axvline(
        3000 + (200 / 25) * 200, linestyle="dotted", linewidth=3, color=colors[1]
    )
    ...
    base_data_dir = os.path.join(
        _parent_dir, "data/agents/cylinder/baseline/checkpoint_000025"
    )
    ...
    dyna_data_dir = os.path.join(
        _parent_dir, "data/agents/cylinder/dyna/checkpoint_000200"
    )
    ...
    df = pd.DataFrame(
        {
            "sindy_med": -10 * dyna_med_eval["r"][-1],
    ...
    plt.plot(
        t,
        df["sindy_med"].rolling(window=win).mean()[:],
        c=colors[1],
        linewidth=LINEWIDTH,
    )
    plt.axvline(0, color="k", linestyle="dashed", linewidth=DOTTED_LINE)
    plt.plot(
        t, df.base_med.rolling(window=win).mean()[:], c=colors[0], linewidth=LINEWIDTH
    )
    ...
    plt.plot(
        t,
        df["sindy_fine"].rolling(window=win).mean()[:],
        c=colors[1],
        linewidth=LINEWIDTH,
    )
    plt.axvline(0, color="k", linestyle="dashed", linewidth=DOTTED_LINE)
    plt.plot(
        t, df.base_fine.rolling(window=win).mean()[:], c=colors[0], linewidth=LINEWIDTH
    )
