...
if __name__ == "__main__":
    ...
    cart_1k_names = {
        ...
        "SINDy-RL (quadratic)": "sindy_rl_quad",
        "SINDy-RL (linear)": "sindy_rl_linear",
        "Dyna-NN": "dyna_nn",
        "MB-MPO": "mbmpo",
    }
    ...
    cart_1k_exp_dirs = {
        key: os.path.join(ray_dir, exp_name) for key, exp_name in cart_1k_names.items()
    }

    # setup dataframes
    cart_1k_df_list_dict = {
        key: get_dfs(exp_dir) for key, exp_dir in cart_1k_exp_dirs.items()
    }

    # setup experiment seed directories
    cart_1k_seed_dirs = {
        key: sorted(glob.glob(os.path.join(exp_dir, "*/")))
        for key, exp_dir in cart_1k_exp_dirs.items()
    }
    ...
    _PLOT_KEYS = {
        "Baseline PPO": "evaluation/episode_reward_mean",
    ...
    # Make plots
    fig, axes = plot_quantile_comparison(
        cart_1k_df_list_dict,
        _PLOT_KEYS,
        _TIME_KEYS,
        _MODE,
        left_ticks=_LEFT_TICKS,
        right_ticks=_RIGHT_TICKS,
        left_lim=_LEFT_LIM,
        alpha=_ALPHA,
        win=win,
        linewidth=3,
