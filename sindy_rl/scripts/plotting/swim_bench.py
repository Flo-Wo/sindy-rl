...
if __name__ == "__main__":
    ...
    swimmer_names = {
        "Baseline PPO + PBT": "baseline_pbt",
        "SINDy-RL + PBT": "sindy_rl_pbt",
    }

    swimmer_exp_dirs = {
        key: os.path.join(ray_dir, exp_name) for key, exp_name in swimmer_names.items()
    }

    swimmer_df_list_dict = {
        key: get_dfs(exp_dir) for key, exp_dir in swimmer_exp_dirs.items()
    }
    swimmer_seed_dirs = {
        key: sorted(glob.glob(os.path.join(exp_dir, "*/")))
        for key, exp_dir in swimmer_exp_dirs.items()
    }
    ...
    LEGENDSIZE = 15
    swimmer_keys = {
        "Baseline PPO + PBT": "episode_reward_mean",
        "SINDy-RL + PBT": "evaluation/episode_reward_mean",
    }

    # Initial points used for data collection before training
    swimmer_t_init = {"Baseline PPO + PBT": 0, "SINDy-RL + PBT": 12e3}

    # Time conversion (since `plot_best_comparison_pbt` uses number of updates)
    swimmer_conversion = {
        "Baseline PPO + PBT": 4e3,  # raw 4000 interactions per RLlib policy updates
        "SINDy-RL + PBT": 1000
        / 5.0,  # collect 1000 on-policy interactions per 5 RLlib policy updates
    }
    ...
    # Make plots
    fig, axes = plot_best_comparison_pbt(
        swimmer_df_list_dict,
        swimmer_keys,
