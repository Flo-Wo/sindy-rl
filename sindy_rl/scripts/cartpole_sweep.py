...
def update_config(config):
    ...
    config["ray_config"]["run_config"][
        "name"
    ] = f"freq={int(_FREQ)}_coll={int(_PI_COLLECT)}_buff={_OFF_BUFFER}_{_ON_BUFFER}_ens-rew=fixed_ens-dyn=med-20_steps=1k"
    return config
...
if __name__ == "__main__":
    ...
    filename = os.path.join(
        _parent_dir, "sindy_rl", "config_templates", "dyna_config_cart.yml"
    )
    ...
    ray.init(address=ip_head, logging_level=logging.ERROR)
    print(ray.nodes())
    ...
    if dyna_config.get("use_pbt", False):
        ...
        pbt_sched = PopulationBasedTraining(**pbt_config, custom_explore_fn=explore)

    ray_config = dyna_config["ray_config"]
    run_config = air.RunConfig(local_dir=LOCAL_DIR, **ray_config["run_config"])

    tune_config = tune.TuneConfig(
        **dyna_config["ray_config"]["tune_config"], scheduler=pbt_sched
    )
    ...
    tune.Tuner(
        tune.with_resources(
            dyna_sindy, drl_class.default_resource_request(drl_default_config)
        ),
