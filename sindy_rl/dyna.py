...
from sindy_rl.logger import get_logger
...
class DynaSINDy(BaseDynaSINDy):
    ...
    def __init__(self, config, logger=None, init_drl=True):
        ...
        self.reset_from_buffer = config["drl"]["config"]["environment"][
            "env_config"
        ].get("reset_from_buffer", False)

        # initialize components
        self._init_real_env()
        print("Real env is set.")
        self._init_off_policy()
        print("Off-policy is set.")
        self._init_data_buffers()
        print("Data buffer is set.")
        self._init_dynamics_model()
        print("Dynamics model is set.")
        self._init_rew_model()
        print("Rew model is set.")
        if init_drl:
            self._init_drl()
            print("\n\nDRL model is set.")
        print("\n\nClass construction was successful.")
    ...
    def _init_drl(self):
        ...
        drl_config_obj = (
            drl_config_obj.rl_module(_enable_rl_module_api=False)
            .environment(**self.drl_config["environment"])
            .training(**self.drl_config["training"], _enable_learner_api=False)
        ...
        self.logger.info("Update of model config worked.")
    ...
    def _init_real_env(self):
        ...
        if isinstance(self.config["real_env"], dict):
            ...
            print("before creating the real environment")
            self.real_env = env_class(env_config)
        else:
            self.real_env = self.config["real_env"]
        self.real_env.reset()
        logging.info("DONE: Init real env.")
    ...
    def _init_dynamics_model(self):
        ...
        logging.info("DONE: init dynamics model.")
    ...
    def _init_rew_model(self):
        ...
        logging.info("DONE: init reward model.")
    ...
    def update_surrogate(self):
        ...
        self.drl_algo.workers.foreach_worker(
            update_dyn_and_rew_models(dyn_weights, rew_weights)
        )
    ...
    def get_buffer_metrics(self):
        ...
        total_samples = (
            self.on_pi_buffer_config["collect"]["n_steps"] * (self.n_dyn_updates - 1)
            + n_samples_off_pi
        )
        ...
        buffer_metrics = {
            ...
            "n_samples": n_samples_on_pi
            + n_samples_off_pi,  # current number of samples between the two buffers
            "n_samples_on_pi": n_samples_on_pi,
            "n_sampels_off_pi": n_samples_off_pi,
            "n_traj": n_traj_on_pi
            + n_traj_off_pi,  # current number of trajectories between the two buffers
