...
class BaseSurrogateEnv(gymnasium.Env):
    ...
    def __init__(self, config):
        ...
        self.use_old_api = self.config.get(
            "use_old_api", False
        )  # True if using old openai gym API. Would not recommend.
    ...
    def _init_act(self):
        ...
        self.action_space = Box(
            low=np.float32(self.act_bounds[0]), high=np.float32(self.act_bounds[1])
        )
    ...
    def _init_obs(self):
        ...
        #  print("bounds internal: ", self.obs_bounds[0], self.obs_bounds[1])

        self.observation_space = Box(
            low=-np.inf * np.ones(self.obs_dim),  # self.obs_bounds[0],
    ...
    def init_real_env(self, env=None, reset=True, **reset_kwargs):
        ...
        """
        print("BaseSurrogateEnv: init_real_env")
        if env:
            self.real_env = env
        else:
            print("BaseSurrogateEnv: init_real_env, second")
            if isinstance(self.real_env_class, str):
                self.real_env_class = getattr(registry, self.real_env_class)

            print("BaseSurrogateEnv: init_real_env, before creation")
            self.real_env = self.real_env_class(self.real_env_config)

        if reset:
            print("BaseSurrogateEnv: init_real_env, before reset call")
