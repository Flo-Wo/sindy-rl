...
def gen_random_then_none(
    ...
    policy = OpenLoopRandRest(steps_rest=n_steps, action_space=env.action_space)

    x_list, u_list, rew_list = rollout_env(
        env, policy, n_steps=n_steps + n_none, verbose=True
    )

    return x_list, u_list, rew_list


def gen_sine_then_none(
    checkpoint,
    env_class=CylinderLiftEnv,
    n_steps=1000,
    n_none=1000,
    ...
    policy = OpenLoopSinRest(
        t_rest, dt=Dt, amp=amp, phase=phase, offset=offset, f0=f0, k=k_int
    )
    ...
    for i in range(n_trajs):
        ...
        try:
            if control_type == "random":
                ...
                x, u, rew = gen_sine_then_none(
                    k_int=1 / (i + 1), seed=seed + i, **kwargs
                )
            else:
...
if __name__ == "__main__":
    ...
    data_dir = os.path.join(_parent_dir, "data/hydro/cylinder/")

    load_data_dir = os.path.join(
        data_dir,
