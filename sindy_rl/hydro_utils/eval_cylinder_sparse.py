...
if __name__ == "__main__":
    ...
    work_dir = os.path.join(
        _parent_dir, "data", "hydro", "cylinder", "eval", "2024-02-10_sparse_filter"
    )
    save_dir = os.path.join(work_dir, f"flow_checkpoints_{MESH}_sparse")

    load_path = os.path.join(work_dir, "spare_policy_filter_trial=0_ckpt=200.pkl")
    ...
    switch_policy = SwitchAfterT(
        t_switch=NULL_STEPS, policies=[null_policy, sparse_policy]
    )
    ...
    obs_list, act_list, rew_list = rollout_env(
        env, switch_policy, n_steps=N_STEPS, env_callback=callback, verbose=True
    )
