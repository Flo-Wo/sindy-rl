...
if __name__ == "__main__":
    ...
    trial_dir = os.path.join(
        exp_dir, f"dyna_sindy_df8d9_{trial_idx:05}_{trial_idx}_2024-02-05_00-09-48"
    )

    ckpt_path = os.path.join(trial_dir, f"checkpoint_{check_idx:06d}")
    ...
    switch_policy = SwitchAfterT(
        t_switch=NO_CONTROL_STEPS, policies=[null_policy, nn_policy]
    )
    ...
    obs_list, act_list, rew_list = rollout_env(
        env, switch_policy, n_steps=CONTROL_STEPS, env_callback=callback, verbose=True
    )
