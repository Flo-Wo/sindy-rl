...
def make_no_control_checkpoints(
    save_dir, mesh, n_steps=10000, render_freq=1000, Re=100, dt=1e-2, make_plot=False
):
    """
    ...
    env_config = {
        "flow": hgym.Cylinder,
        "flow_config": {"mesh": mesh, "Re": Re, "actuator_integration": "implicit"},
        "solver": hgym.IPCS,
        "solver_config": {"dt": dt},
    }
...
def produce_checkpoints(
    ...
    for i in tqdm(range(n_steps)):
        ...
        if (i % render_freq) == (render_freq - 1):
            if make_plot:
                save_plot(env, idx=i, fname=fname + ".png", dir_name=dir_name)
